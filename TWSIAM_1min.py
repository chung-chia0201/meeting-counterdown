...
#倒數計時模式
speech_model=[60,15,5,-10]                   #60\15\5\-10
model_time=int(speech_model[0])-int(speech_model[3])

#字體大小
word_size=int(22*mult)*mult_compare
word_title_size=int(42*mult)*mult_compare
line_space_size=int(10*mult)*mult_compare
number_size=int(400*mult)*mult_compare                
    ...
    while waiting:
        ...
        for event in pg.event.get():           
            if event.type == pg.QUIT:         #如果按叉叉就把視窗關掉
                ...
                waiting=False
                return False
            elif event.type == pg.MOUSEBUTTONUP:     #如果按滑鼠a就進入主畫面
                waiting=False
                return False
...
while running: 
    ...
    start_sec=time_start.tm_hour*3600+time_start.tm_min*60+time_start.tm_sec+hold_sec      #改
    hold_remaining_sec=start_sec-now_sec                                                   #改
    speech_time=int(model_time) #這個活動的時間有多長   
    remaining_time_sec=start_sec-now_sec+int(speech_model[0])+model_time*count  #一開始count=0
    if remaining_time_sec<=speech_model[3]:
        count+=1
    ...
    #畫面顯示
    if hold_remaining_sec<=hold_sec and hold_remaining_sec>0:                   #改
        screen.fill(WHITE)
        hold_remaining_sec=str(hold_remaining_sec)                              #改
        if len(hold_remaining_sec)==1:
            hold_remaining_sec='0'+hold_remaining_sec
        hold_remaining_time='00:'+hold_remaining_sec
        draw_text(screen,str(hold_remaining_time),number_size,WIDTH/2-number_size*(5/4),HEIGHT/2-number_size*(5/8),remaining_number_color_normal)   
    ...
    elif remaining_time_sec<=speech_remaing_time2 and remaining_time_sec>speech_remaing_time3:
    ...
    elif remaining_time_sec<=speech_remaing_time3 and remaining_time_sec>0:
        ...
        remaining_time_sec=-1*int(speech_remaing_time4)+remaining_time_sec
        x=str(int(remaining_time_sec)//60)
        y=str(int(remaining_time_sec)%60)
        if len(x)==1: 
            x='0'+x
        if len(y)==1:
            y='0'+y
        remaining_time=x+':'+y
        screen.fill(backgroung_color_countdown3)
        # draw_text(screen,str(remaining_time),number_size,WIDTH/2-number_size*(5/4),HEIGHT/2-number_size*(5/8),remaining_number_color_countdown3) 
        slogan_word_size=int(number_size*(8/10))                                                                                                #(改)
        draw_text(screen,slogan2,slogan_word_size,WIDTH/2-slogan_word_size*(3/2),HEIGHT*(1/3)-slogan_word_size*(1/2),remaining_number_color_countdown3)   #時間到了

        slogan_word_number_size=int(HEIGHT-(HEIGHT*(1/3)-slogan_word_size*(1/2))-slogan_word_size-4*line_space_size)
        draw_text(screen,str(remaining_time),slogan_word_number_size,WIDTH/2-slogan_word_number_size*(5/4),HEIGHT-3*line_space_size-slogan_word_number_size,remaining_number_color_countdown3)   #數字  
        
    all_sprites.draw(screen)
    draw_text(screen,str(now_moment),word_size+15,line_space_size,line_space_size,word_color)  #改
    pg.display.update()
# print(word_size,word_title_size,line_space_size,number_size)
pg.quit()
