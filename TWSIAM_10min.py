...
while running: 
    ...
    start_sec=time_start.tm_hour*3600+time_start.tm_min*60+time_start.tm_sec+hold_sec
    hold_remaining_sec=start_sec-now_sec
    speech_time=int(model_time) #這個活動的時間有多長   
    remaining_time_sec=start_sec-now_sec+int(speech_model[0])+model_time*count  #一開始count=0
    if remaining_time_sec<=2*speech_model[3]:
        count+=1
    ...
    if hold_remaining_sec<=hold_sec and hold_remaining_sec>0:                   #改
    ...
    elif remaining_time_sec<=0 and remaining_time_sec>-speech_remaing_time3:  #改這裡
        remaining_time_sec=start_sec-now_sec+int(speech_model[0])+model_time*count
        remaining_time_sec=int(speech_remaing_time3)+remaining_time_sec
        x=str(int(remaining_time_sec)//60)
        y=str(int(remaining_time_sec)%60)
        if len(x)==1:
            x='0'+x
        ...
        # slogan_word_number_size=int((HEIGHT*(1/3)-slogan_word_size*(1/2))+slogan_word_size+line_space_size)
        ...
        remaining_time_sec=start_sec-now_sec+int(speech_model[0])+model_time*count-int(speech_model[3])
